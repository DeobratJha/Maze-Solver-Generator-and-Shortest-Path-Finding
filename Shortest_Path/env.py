...
class Board:
    ...
    def __init__(self, v_cells:int, h_cells:int, origin_x:int, origin_y:int, 
        ...
        self.start = None
        self.target = None
        self.sky_blue_walls = set()

    
  

    
# Add this in your class __init__ method to initialize sky_blue_walls attribute
# import random
...
#self.sky_blue_walls = set()
    ...
    def draw_board(self, return_cells=True) -> bool: 
        ...
        # Set the sky-blue wall cells if it hasn't been done already
        if not self.sky_blue_walls and len(self.wall) >= 15:
            self.sky_blue_walls = set(random.sample(self.wall, 15))
        ...
        for i in range(self.v_cells):
            ...
            for j in range(self.h_cells):
                ...
                if (i, j) == self.start:
                    ...
                    # Use sky blue for the selected walls, gray for others
                    color = self.colors["sky_blue"] if (i, j) in self.sky_blue_walls else self.colors["gray"]
